...
def meeting(s):
    ...
    for x in range(len(name_list)):
        name_list[x]=name_list[x].split(":")
        name_list[x][0], name_list[x][1] = name_list[x][1].upper(), name_list[x][0].upper()
        name_list[x] = ", ".join(name_list[x])
        name_list[x] = "({})".format(name_list[x])
    name_list.sort()
